...
function deadlineLabel(iso: string) {
  ...
  if (diff < 0)  return { text: 'Overdue',       color: 'text-[#f06060]' };
  if (diff === 0) return { text: 'Due today',    color: 'text-[#f0b429]' };
  if (diff <= 3)  return { text: `${diff}d left`, color: 'text-[#f0b429]' };
  return { text: `${diff}d left`, color: 'text-[#2ecb7a]' };
}
    ...
    <div>
      ...
      <div className="sticky top-0 z-10 bg-[#0d0f12] border-b border-[#272c3a] px-8 py-5
        ...
        <div>
          ...
          <p className="text-sm text-[#7a80a0] mt-1">{labs.length} lab assignments</p>
        </div>
      ...
      <div className="p-8 space-y-4">
        {labs.map(lab => {
          ...
          return (
            <div key={lab.id}
                 className="bg-[#141720] border border-[#272c3a] rounded-xl p-5
                            hover:border-[#f0b42940] transition-all">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-lg bg-[#f0b429]/10 flex items-center justify-center flex-shrink-0">
                    <FlaskConical size={18} className="text-[#f0b429]"/>
                  </div>
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-bold text-[15px] text-white">{lab.title}</span>
                      <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full
                        ${lab.isPublished
                          ? 'bg-[#2ecb7a]/10 text-[#2ecb7a]'
                          : 'bg-[#7a80a0]/10 text-[#7a80a0]'}`}>
                        {lab.isPublished ? 'Published' : 'Draft'}
                      </span>
                    </div>
                    <p className="text-[13px] text-[#7a80a0] mt-1 max-w-xl">{lab.description}</p>

                    <div className="flex items-center gap-5 mt-3 text-[12.5px] text-[#7a80a0]">
                    ...
                    {/* Progress bar */}
                    <div className="mt-3 w-64 h-1.5 bg-[#272c3a] rounded-full overflow-hidden">
                      <div className="h-full bg-[#f0b429] rounded-full transition-all"
                           style={{ width: `${pct}%` }}/>
                    </div>
                    <div className="text-[11px] text-[#7a80a0] mt-1">{pct}% submission rate</div>
                  </div>
                </div>

                <div className="flex gap-2 flex-shrink-0">
                  <Link href={`/teacher/submissions?lab=${lab.id}`}
                        className="border border-[#272c3a] text-[12px] font-semibold px-3 py-1.5
                                   rounded-lg hover:border-[#f0b429] hover:text-[#f0b429] transition-colors text-white">
                    View Submissions
                  </Link>
                  <Link href={`/teacher/create?edit=${lab.id}`}
                        className="border border-[#272c3a] text-[12px] font-semibold px-3 py-1.5
                                   rounded-lg hover:border-[#3d8ef0] hover:text-[#3d8ef0] transition-colors text-white">
                    Edit
                  </Link>
                </div>
              </div>
